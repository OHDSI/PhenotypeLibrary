...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (437082,75347,4153359,73562)
...
AND PR.specialty_concept_id in (38003882,38003957,4147272,44777791,4163857,3191718,38004491,3196458)
AND C.ordinal = 1
...
LEFT JOIN @cdm_database_schema.PROVIDER PR on C.provider_id = PR.provider_id
WHERE PR.specialty_concept_id in (38003882,38003957,4147272,3196458,44777791,4163857,3191718,38004491)
-- End Condition Occurrence Criteria
...
WHERE DATEDIFF(d,C.observation_period_start_date, C.observation_period_end_date) >= 365
