...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1305058,42899196,1777087,19010482,964339,19014878,1708880,19028050,1101898,1114375,1119119,1151789,937368)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (1305058,42899196,1777087,19010482,964339,19014878,1708880,19028050,1101898,1114375,1119119,1151789,937368)
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,180,P.START_DATE) AND A.START_DATE <= P.OP_END_DATE
...
WHERE PR.specialty_concept_id in (4202807,4163857,38003882,38003957,3198921,44777791,3191718,38004491)
-- End Condition Occurrence Criteria
