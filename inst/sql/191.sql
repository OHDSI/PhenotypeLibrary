...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4223659,4272240,4061577,442165)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4223659,4272240,4061577,442165)
...
) A on A.person_id = P.person_id  AND A.START_DATE >= DATEADD(day,-14,P.START_DATE) AND A.START_DATE <= DATEADD(day,-1,P.START_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
