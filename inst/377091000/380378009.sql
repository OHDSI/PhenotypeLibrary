...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (376680,40480055,374924,43530734,381842,373748,374915,4309257,4165417,43530626,40483317,37109663,4306496,374023,4311977,4029498,380378,377091)

) I
) C UNION ALL 
SELECT 1 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (734275,740910,740275,745466,797399,705103,711584,742267,718122)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (734275,740910,740275,745466,797399,705103,711584,742267,718122)
...
WHERE YEAR(C.condition_start_date) - P.year_of_birth >= 66
AND C.ordinal = 1
