...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (439703,439702,439776,43020503)
...
WHERE (YEAR(C.condition_start_date) - P.year_of_birth >= 2 and YEAR(C.condition_start_date) - P.year_of_birth <= 20)
-- End Condition Occurrence Criteria

  ) E
	JOIN @cdm_database_schema.observation_period OP on E.person_id = OP.person_id and E.start_date >=  OP.observation_period_start_date and E.start_date <= op.observation_period_end_date
  WHERE DATEADD(day,180,OP.OBSERVATION_PERIOD_START_DATE) <= E.START_DATE AND DATEADD(day,180,E.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE
) P
WHERE P.ordinal = 1
-- End Primary Events
