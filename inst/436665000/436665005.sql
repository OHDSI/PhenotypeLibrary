...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (440078,439256,433992,443797,441834,439255,439254,437528,442570,436386,439253,439251,435226,439248,372599,439246,439249,437529,439245,432876,436665,438727,440980,4148842,432866,432290,440067,4220617,4327669,443237,437249,439272,438405,438119,439262,437831)

) I
) C UNION ALL 
SELECT 2 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4152280,435783,4286201,374009)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4152280,435783,4286201,374009)
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (751246,745466,19124477,735979,785788,766814,712615,757688,740275)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (751246,745466,19124477,735979,785788,766814,712615,757688,740275)
  and c.invalid_reason is null
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,0,P.START_DATE) AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,0,P.END_DATE) AND A.visit_occurrence_id = P.visit_occurrence_id
...
WHERE PR.specialty_concept_id in (38003629,38003631,38003634,38003638,38003639,38003650,38003653,38003774,38003979,43125860,38003983,38003994,38004469,38004383,38004408,38004409,38004414,38004488,38004493,44777741,38004505,45756756)
-- End Visit Occurrence Criteria
